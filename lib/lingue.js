...
// Il codice esteso che vogliono la lettura ad alta voce e il microfono: "it"
// da solo non basta, ci vuole anche il paese. Sono gli stessi codici usati dal
// riconoscitore di lingua in pages/diagnosi.jsx, e devono restare allineati:
// se qui comparisse "en-US" e li' "en-GB", la voce non verrebbe mai trovata.
const VOCI = {
  it: "it-IT",
  en: "en-GB",
  es: "es-ES",
...
export function voceDi(lingua) {
  return VOCI[lingua] || VOCI[PREDEFINITA];
}

/**
 * Il pezzo da mettere davanti agli indirizzi interni: "" per l'italiano,
 * "/es" per lo spagnolo.
 *
 * Serve perche' il sito naviga con <a href="/diagnosi"> e non con il <Link>
 * di Next.js: un <a> normale ricarica la pagina e NON si porta dietro la
 * lingua, quindi dalla home spagnola si finiva sulla diagnosi italiana.
 *
 * Si potrebbe risolvere passando a <Link>, che la lingua la conserva da solo.
 * Non si fa di proposito: <Link> naviga senza ricaricare e cambia il modo in
 * cui si impila la cronologia — cioe' proprio la cosa costata cinque prove e
 * tre diagnosi sbagliate col tasto indietro, a settembre 2026. Il prefisso
 * ottiene lo stesso risultato senza toccare niente di tutto quello.
 */
export function prefissoDi(lingua) {
  return !lingua || lingua === PREDEFINITA ? "" : `/${lingua}`;
}
