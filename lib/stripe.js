...
// Senza chiave si costruisce comunque, con un segnaposto. Sembra permissivo,
// ed e' il contrario: serve perche' "npm test" possa caricare i moduli che
// parlano con Stripe SENZA avere in mano le chiavi vere. Le prove sostituiscono
// le chiamate con delle finte (vedi test/bonifico.test.mjs), quindi questa
// chiave non viene mai usata davvero. Se invece manca in produzione, la prima
// chiamata vera fallisce subito con un errore di autenticazione chiaro.
if (!process.env.STRIPE_SECRET_KEY) {
  console.warn("STRIPE_SECRET_KEY mancante: le chiamate a Stripe falliranno.");
}

export const stripe = new Stripe(process.env.STRIPE_SECRET_KEY || "sk_test_chiave_assente", {
  ...
  //
  // Era "2024-06-20", scritta a memoria. Stripe ha risposto che e' vecchia:
  // l'attuale e' questa. Il promemoria vale piu' della data — su Stripe le
  // cose cambiano, e quello che "si sa" va verificato contro l'API vera.
