...
export function refertoPDF(report, appliance, brand, problem) {
  const { doc, nomeFile } = costruisciReferto(report, appliance, brand, problem);
  return { blob: doc.output("blob"), nomeFile };
}

function costruisciReferto(report, appliance, brand, problem) {
  ...
  doc.text(`Referto #${numero}`, W - margin, 20, { align: "right" });
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text("Fixi — Diagnosi elettrodomestici via AI", margin, pageH - 12);
